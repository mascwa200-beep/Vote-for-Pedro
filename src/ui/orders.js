...
import {
  intentHelp, phraseCount, INTENTS, STATION_AFFINITY, readWeaponType,
} from '../lang/lexicon.js';
...
const ORDERS = [
  ...
  },
  {
    ...
    build: (t) => ({
      action: 'fire',
      // "Guessing consistently is better than guessing differently in two
      // places" is what this used to say, above a second copy of the rule that
      // guessed differently from the lexicon's on seven of the fleet's weapon
      // names. There is one copy now, and this is it.
      weaponType: readWeaponType(t),
    }),
  },
 ...
 *
 * `shortNames` relaxes the one-word rule below, and only the training reading
 * passes it — see the call site.
 */
function matchAbility(t, { shortNames = false } = {}) {
  for (const a of Object.values(ABILITIES)) {
    ...
    if ((shortNames || name.includes(' ')) && t.includes(name)) return a.id;
  }
  return null;
}
...
export function parseOrder(raw, crew = null) {
  ...
  if (TRAINING.test(t)) {
    // "Train X" names an ability outright, and that is the one context where a
    // one-word name is not ambiguous. The rule in `matchAbility` exists so a
    // bare name cannot swallow a sentence that merely contains it — "brace the
    // port shields" is an order to a shield facing — but nothing else can be
    // meant by "train stimulants", and Stimulants is the only ability in the
    // game with a one-word name.
    //
    // The Training screen prints `train ${a.name.toLowerCase()}` on every
    // button it offers, and that is the whole point of a `say:` line: it
    // teaches the words. For this one ability the words it taught did nothing.
    const trainee = ability ?? matchAbility(t, { shortNames: true });
    if (trainee) return to({ action: 'train', ability: trainee });
  }
  if (ability) {
...
/**
 * Sayable examples for orders whose lexicon phrasings are all sentence stems.
 *
 * The lexicon's phrase list is a MATCHING list: `power` carries nineteen of
 * them and every one is a fragment — "give me more power", "reallocate power",
 * "take power from" — because the channel is read separately. That is right for
 * matching and wrong for a manual. The reference printed four of those stems,
 * and all four answer "Power to which system, Captain?", so the one screen
 * whose whole job is to teach phrasings taught four fragments.
 *
 * One order needs this today. The test below is what keeps it that way: no
 * entry may print only examples that cannot be said as they stand.
 */
const SAYABLE_EXAMPLES = {
  power: [
    'divert power to shields', 'more power to the engines',
    'all power to weapons', 'reroute power to the shields',
  ],
};
...
export function commandReference({ examples = 4 } = {}) {
  ...
  for (const intent of INTENTS) {
    push(STATION_OF[intent.id] ?? 'command', {
      ...
      // are the most natural-sounding ones — which is what to show. Unless they
      // are all stems, in which case what to show is written out by hand.
      examples: (SAYABLE_EXAMPLES[intent.id] ?? intent.phrases).slice(0, examples),
