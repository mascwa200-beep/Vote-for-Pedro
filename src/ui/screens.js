...
import { listBackups, downloadSave, listSaves, deleteSave } from '../core/save.js';
...
export function bridgeScreen(app) {
  ...
  if (walking) {
    ...
    // A survey already done. `surveyFeature` has refused a second one since it
    // was written — "We have already been over that one, Captain." — and
    ...
    // of them mean the away team has been over it.
    const done = survey && !!g.surveyed?.[`${w.room.world}:${target.id}`];
    hand.push(button(
      survey ? `Survey ${lowerFirst(target.label)}`
        : target.panel || target.id ? `Use ${target.label ?? ROOMS[target.to]?.name}` : 'Use',
      done ? null : tap(() => app.useWhatIsInFront()),
      {
        color: done ? 'ghost' : 'orange',
        disabled: !!done,
        sub: done
          ? 'We have already been over that one'
          : survey
  ...
  if (!walking && doors.length) {
    // Whether the ship will let the captain leave the room at all, asked ONCE
    // for the row rather than per door: `mayWalk` is a fact about the ship, not
    // about which bulkhead you are pointed at.
    //
    // Every one of these was drawn blue during a fight and did NOTHING. The
    // handler read `.ok` and threw the reason away, so tapping a door under
    // fire produced no message, no log entry and not even a deny cue — the
    // chip simply absorbed the tap. The rule is real and worth keeping; being
    // told about it is the part that was missing.
    const may = g.mayWalk?.() ?? { ok: true };
    hand.push(el('div', { class: 'chip-row' }, doors.slice(0, 6).map((e) => button(
      ROOMS[e.to]?.name ?? e.to,
      may.ok ? tap(() => { if (g.goToRoom(e.to).ok) audio.play('door'); app.render(); }) : null,
      {
        color: may.ok ? 'blue' : 'ghost',
        disabled: !may.ok,
        say: `take me to the ${(ROOMS[e.to]?.name ?? e.to).toLowerCase()}`,
      },
    ))));
    // Said once under the row, for the same reason it is asked once: six chips
    // each carrying the same sentence is a wall of text about one rule.
    if (!may.ok) hand.push(el('p', { class: 'hint', text: may.reason }));
  }
...
export function orbitPanel(app) {
  ...
  wrap.append(panel('Standard Orbit', [
    ...
    // The return value was discarded outright, so breaking orbit while ashore
    // was a button that accepted the tap and did nothing at all. `breakOrbit`
    // has always refused it, and always had the right sentence for why: "You
    // are on the surface, Captain. We are not leaving without you."
    button('Break orbit', g.ashore ? null : tap(() => {
      const r = g.breakOrbit();
      if (!r.ok) app.showMessage('Helm', [r.error ?? r.reason]);
      app.render();
    }), {
      color: 'ghost',
      disabled: !!g.ashore,
      sub: g.ashore ? 'Not with the captain on the surface' : undefined,
      say: 'break orbit',
    }),
  ], 'accent'));
...
export function tacticalScreen(app) {
  ...
  side.append(panel('Target Subsystem', [
    ...
    el('p', { class: 'hint', text: 'Targeting a subsystem trades raw damage for a specific outcome, and the right target changes with the enemy: their warp core or engines to stop a runner, their weapons to survive a slugger like a Galor — where crippling the engines instead is the wrong call and costs you the fight. It costs fifteen percent of the whole shot, shields as much as hull, and it only cripples in proportion to what reaches the hull, so call it once their shields are down. Which is what they do to you.' }),
  ]));
  ...
  side.append(panel('Helm', [
    ...
    el('div', { class: 'grid-2' }, [
      ...
      // The cost, said out loud. Evasive is `defense + 0.16`, `maxSpeed x 1.25`
      // and `turnRate x 1.4` — three benefits and no declared price, which made
      // it look free in the source. It is not: a ship jinking that hard cannot
      // hold a firing arc, so its own gunnery falls off. Measured across six
      // blocks of thirty runs, holding evasive cost kills in 6 of 6 and left
      // more hull on the survivors in 6 of 6. A real trade the button never
      // mentioned, next to a targeting panel that has explained its own since
      ...
      button(g.ship.evasive ? 'Evasive: on' : 'Evasive', tap(() => {
        ...
        sub: 'Harder to hit, harder to shoot: you will take less and kill slower.',
      }),
    ]),
...
export function optionsScreen(app) {
  ...
  //
  // `listSaves` and `deleteSave` have been written, complete and exported since
  // the save system was built, and nothing imported either of them. So the slot
  // machinery existed — an autosave and three named slots, each with a label
  // and a timestamp — and a captain had no way to see what was in them or to
  // clear one. The file's own header argues at length about a save surviving
  // "five years of storage pressure, a phone running out of space mid-write",
  // and then gave the player no way to manage the storage.
  //
  // Deleting is behind a confirm, because it is the one button on this screen
  // that destroys a commission.
  const saves = listSaves();
  root.append(panel('Saved Commissions', [
    saves.length
      ? el('div', {}, saves.flatMap((sv) => [
        el('p', { class: 'muted', text: `${sv.slot === 'auto' ? 'Autosave' : `Slot ${sv.slot}`}: `
          + `${sv.label ?? 'unnamed'} — ${new Date(sv.savedAt).toLocaleString()}` }),
        button(`Delete ${sv.slot === 'auto' ? 'the autosave' : `slot ${sv.slot}`}`, tap(() => {
          // Through `showMessage`'s own actions, which is how this app asks a
          // question — the same shape as `confirmNewGame` in `main.js`: the
          // destructive answer in red, the way out in ghost, and no spoken
          // phrase on either. The first draft reached for a confirm helper that
          // does not exist on `app`, and the optional call fell straight
          // through to deleting a five-year commission on one tap.
          const what = sv.slot === 'auto' ? 'the autosave' : `slot ${sv.slot}`;
          app.showMessage('Delete a saved commission', [
            `Delete ${what} — ${sv.label ?? 'unnamed'}?`,
            sv.slot === 'auto'
              ? 'This also clears the three autosave backups behind it. It cannot be undone.'
              : 'It cannot be undone.',
          ], [
            // Neither of these advertises a spoken phrase. `showMessage` only
            // registers a voice route for its DEFAULT action — see the
            // `if (!actions)` in `main.js` — so a modal carrying its own
            // buttons cannot be answered by speaking, and a phrase printed here
            // would be one more promise the game does not keep.
            button('Delete it', () => { deleteSave(sv.slot); app.closeModal(); app.render(); },
              { color: 'red' }),
            button('Cancel', () => { app.closeModal(); }, { color: 'ghost' }),
          ]);
        }), { color: 'red', sub: sv.slot === 'auto' ? 'Also clears the three backups' : 'Cannot be undone' }),
      ]))
      : el('p', { class: 'muted', text: 'Nothing saved yet. The game keeps an autosave as you play.' }),
  ]));
