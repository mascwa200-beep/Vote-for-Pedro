...
export function resolve(rng, {
  ...
  // Rounded first, and every verdict derived from the rounded value.
  //
  // This ordering is load-bearing. Reporting a margin of -0.02 as "0.0" while
  // deriving failure from the raw number means the card shows the player a
  // number that contradicts its own verdict — and because JavaScript evaluates
  // `-0 >= 0` as true, it is not even visible as a sign. It happened about one
  // attempt in a hundred and CI caught it before a player could.
  const raw = BASELINE + capability + swing + luck - difficulty;
  const margin = Math.round(raw * 10) / 10 || 0;   // `|| 0` normalises -0
