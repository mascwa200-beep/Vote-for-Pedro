...
export function drawCombatEffects(renderer, engagement, opts = {}) {
  ...
  for (const e of engagement.effects ?? []) {
    ...
    // Against the flare's own span, for the reason the explosion pass records
    // below: the renderer holding a second copy of a number the simulation owns
    // is a convention, and the explosion is proof the convention does not hold.
    // One caller today at 0.4, agreeing by luck rather than by construction.
  ...
  for (const e of engagement.effects ?? []) {
    ...
    // Same again. `1 - e.life` is `1 - e.life / 1.0` with the divisor written
    // as an assumption instead of as a value, which is the form the explosion
    // was in when three of its four callers disagreed with it.
    const age = clamp(1 - e.life / (e.span || 1), 0, 1);
