...
export class Game {
  ...
  intercom(dept) {
    ...
    const reports = {
      ...
      // `warpFactor`, not `factor`. `Transit` has never had a `factor` — the
      // constructor takes `warpFactor` and stores it under that name — so
      // "helm report" threw on every ship that was actually under way, for the
      // whole life of the intercom. Invisible because it needs the one order
      // given in the one state; found by the order monkey in
      // tools/verify-app.mjs, which gives every phrasing at every moment.
      helm: () => (this.transit
        ? `Underway for ${this.transit.to?.name ?? 'the next system'}, `
          + `warp ${(this.transit.warpFactor ?? 1).toFixed(1)}.`
        : `Station keeping at ${this.location?.name ?? 'these coordinates'}. `
          + `Engines at ${pct(s.subsystems.engines)}.`),
   ...
   */
  /**
   * What you get for watching a patrol instead of hailing it.
   *
   * Keyed by the errand the encounter already chose. `PATROL_ERRANDS` in
   * world/encounters.js says what they are doing; this says what a careful
   * look at it is worth, which is the other half of the same sentence.
   */
  static PATROL_WATCH = {
    scanning: { seen: 'They are mapping our shield harmonics, politely, and logging the result. We log theirs.' },
    border_run: { seen: 'The cutter runs the line to a hundredth of a degree. Navigation now has the line to a hundredth of a degree.' },
    tender: { seen: 'Buoy servicing, done well. Their escort has a sensor blind spot aft and does not know it.' },
    search: { seen: 'The pattern is a search for wreckage, not a ship. Whatever they lost, they lost it badly.' },
    convoy_screen: { seen: 'Something large is behind them running dark. We do not get a profile, but we get a mass.' },
    relief: { seen: 'Relief supplies, and the manifest is on an open channel because nobody expected anyone to read it.' },
    survey: { seen: 'A standard grid, worked properly. Science quietly copies their gridding method.' },
    default: { seen: 'They go about their business and we go about ours, both of us watching.' },
  };
  ...
  encounterChoices() {
    ...
    switch (enc.kind) {
      ...
      case 'patrol':
        ...
        // Watching is always available, and is the only thing available at all
        // when the faction does not answer hails. Measured over twenty
        // thousand encounters, 109 of them — every non-hostile patrol from the
        // Dominion, the Tholians or the Borg — offered a single button reading
        // "Continue", which is not an encounter, it is a notification.
        //
        // And `subtype` has carried a real errand since the errand table was
        // written: a tender servicing navigation buoys, a destroyer screening
        // something you cannot see, a squadron quartering the system for
        // something it will not name. Nothing had ever read it back.
        add('observe', 'Watch what they are doing', 'observe them',
          'Passive sensors, and a line in the log.', 'ice');
  ...
  resolveEncounter(choiceId) {
    ...
    switch (choiceId) {
      ...
      // Watching a patrol go about its business. Not `scan`, which catalogues an
      ...
      case 'observe': {
        ...
        if (this.rng.chance(quality)) {
          ...
          this.awardXP(30);
          out.messages.push('They are careful. We learn very little, and they know we tried.');
        }
        // Through `endEncounter`, not by hand. The first draft of this cleared
        // `this.encounter`, stood the alert down and emitted the event — and
        // forgot the one line that puts the mode back, leaving the game in
        // ENCOUNTER mode with no encounter in it. The order monkey in
        // tools/verify-app.mjs found it within one run: "no order throws,
        // whenever it is given — helm report".
        this.endEncounter();
        return out;
      }
  ...
  startCombat(hostiles, opts = {}) {
    ...
    // The other side of it: being jumped.
    //
    // `buildAmbush` has set `surprise: true` since encounters were written and
    // NOTHING had ever read it, so "Sensors read nothing — then everything.
    // Ships decloaking off both bows" opened exactly like a routine patrol,
    // with every gun on both sides ready. Measured through `resolveEncounter`:
    // patrol and ambush came back byte for byte identical.
    //
    // Symmetrical with `first_strike` on purpose, and in the same place: one
    // side opens a cycle behind, which is a real free volley and the whole
    // meaning of the word.
    //
    // Two species sell a defence against this and neither had anything to
    // defend against. The Caitian's Predator's Instinct — "You always act
    // first in an engagement, and can never be surprised" — cancels it
    // outright; the Saurian's Wide Spectrum Vision — "Advantage against
    // ambushes" — halves it, because seeing it coming is not the same as
    // having been ready.
    if (opts.surprise) {
      const immune = this.character?.mechanic('surpriseImmune')
        || this.character?.mechanic('alwaysFirst');
      const warned = this.character?.mechanic('ambushAdvantage');
      if (immune) {
        this.pushLog('You saw them before they moved. We are already turning.', 'tactical');
      } else {
        const share = warned ? 0.5 : 1;
        for (const w of this.ship.weapons) w.cooldown = Math.max(w.cooldown, w.cycle * share);
        this.pushLog(warned
          ? 'Contact — but we had them a half-second early. Guns are coming round.'
          : 'They were waiting for us, Captain. We are late to our own fight.', 'tactical');
      }
    }
  ...
  awayMission(templateId, opts = {}) {
    ...
    // The people who did not walk back onto the transporter pad.
    //
    // A landing party's casualties are named officers, who are killed and
    // injured on their own objects, and anonymous security crewmen, who are
    // not. The anonymous half reached NOTHING: measured over 260 landings, 12
    // of 60 reported a crewman hurt and `ship.injured` moved on none of them,
    // and the two that reported one killed left the complement at 430.
    //
    // `ship.injured` is what the crew screen renders as "N in sickbay", and it
    // is the count for exactly this population — officers are listed separately
    // by name. So a party came back, the report said a man was hurt, and the
    // captain could walk to sickbay and find it empty.
    //
    // Counted here rather than in `away.js` because that module resolves the
    // landing and does not hold the ship. Named officers are excluded: they are
    // already accounted for as themselves, and adding them here would count
    // each one twice.
    const aboard = new Set(this.crew.officers.map((o) => o.name));
    const anonymous = team.casualties.filter((c) => !aboard.has(c.name));
    const anonKilled = anonymous.filter((c) => c.killed).length;
    const anonHurt = anonymous.filter((c) => c.injured && !c.killed).length;
    if (anonKilled) this.ship.crew = Math.max(0, this.ship.crew - anonKilled);
    if (anonHurt) this.ship.injured += anonHurt;
    ...
    // And say who is in sickbay, which this never did. It reported the dead
    // and nothing else, so the twenty percent of landings that cost somebody an
    // injury read in the log as landings that cost nothing at all.
    const hurtAboard = team.casualties.filter((c) => c.injured && !c.killed).length;
    this.pushLog(
      `Landing party is back aboard. ${won} of ${total} objectives.`
      + (lost ? ` We lost ${lost}.` : '')
      + (hurtAboard ? ` ${hurtAboard} for sickbay.` : ''),
      'transporter',
    );
