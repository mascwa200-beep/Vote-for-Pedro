...
/**
 * A hull number for a new command, which is not the one you just lost.
 *
 * `Ship` falls back to `FEDERATION_REGISTRIES[0]` when given no registry, and
 * that is `NCC-1701`. So every Federation hull the captain was ever handed
 * carried the same number, and nine of the ten registries in that list had
 * never been used by anything. Losing the Enterprise and being given a
 * Constellation produced `USS Lexington NCC-1701` — a fresh name on the dead
 * ship's number, which is the one thing a registry is for.
 *
 * `takeCommandOf`'s own comment already says what should happen — promotion and
 * a board of inquiry "get a fresh hull with a fresh name" — and the name half
 * was the only half that happened.
 *
 * Worth knowing: this is why #116's "the yard keeps her number" assertion was
 * vacuous. It compared `NCC-1701` against `NCC-1701` and passed without the fix
 * it was written for, because every fresh hull carried that number.
 */
function registryFor(game, avoid) {
  const pool = FEDERATION_REGISTRIES.filter((r) => r !== avoid);
  if (!pool.length) return FEDERATION_REGISTRIES[0];
  return pool[Math.floor(game.rng.float() * pool.length) % pool.length];
}
...
export function takeCommandOf(game, classId, { name = null, registry = null } = {}) {
  ...
  game.ship = new Ship(classId, {
    ...
    // board of inquiry pass neither and get a fresh hull with a fresh name AND
    // a fresh number, which is what those are — the number used to fall through
    // to `Ship`'s default and come out as the registry of the ship just lost.
    registry: registry ?? registryFor(game, game.ship?.registry ?? null),
    faction: 'federation',
    isPlayer: true,
  });
